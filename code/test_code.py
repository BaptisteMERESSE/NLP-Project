...
# 1. Créer une palette de couleurs fixe pour les thèmes pour assurer la cohérence
unique_themes = df_map['Nom_Theme'].unique()
theme_palette = dict(zip(unique_themes, sns.color_palette("tab20", len(unique_themes))))

# 2. Préparer la grille (2 lignes, 3 colonnes pour accueillir les 5 partis)
...
for i, parti in enumerate(partis_principaux):
    ...
    # A. Le masque : On affiche TOUS les points en gris clair en arrière-plan
    sns.scatterplot(
        data=df_plot, x='x', y='y', 
        color='lightgray', s=30, alpha=0.3, edgecolor='none', ax=ax, legend=False
    )
    
    # B. Le focus : On affiche UNIQUEMENT les points du parti au premier plan
    ...
    # Nettoyage visuel de chaque sous-graphique
...
# 3. Masquer le 6ème graphique vide (puisqu'on a un Top 5)
if len(partis_principaux) == 5:
    plt.subplot(2, 3, 6).axis('off')
    
    # On peut utiliser cet espace vide pour mettre la légende globale
